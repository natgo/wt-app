import { Arrow } from "../Arrow";
import { EmptyDiv } from "../EmptyDiv";
import { MediaHead } from "../MediaHead";
import { Rank } from "../Rank";
import { Research } from "../Research";
import { TechTreeItem } from "../TechTreeItem";
import { TreeFolder } from "../TreeFolder";

export function USSRGround() {
  return (
    <div className="mw-parser-output">
      <div className="tree">
        <table rules="rows" width="100%">
          <tbody>
            <Research research={5} premium={2} />
            <tr>
              <Rank rank="I" />
              <td>
                <TechTreeItem
                  link="/BT-5"
                  title="BT-5"
                  item_type="own"
                  text="BT-5"
                  intname="ussr_bt_5"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/BT-7"
                  title="BT-7"
                  item_type="own"
                  text="BT-7"
                  intname="ussr_bt_7_1937"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/BT-7M"
                  title="BT-7M"
                  item_type="own"
                  text="BT-7M"
                  intname="ussr_bt_7_m"
                  type="ground"
                />
                <Arrow length={2} />
              </td>
              <td>
                <TechTreeItem
                  link="/T-26"
                  title="T-26"
                  item_type="own"
                  text="T-26"
                  intname="ussr_t_26_1940"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-26-4"
                  title="T-26-4"
                  item_type="own"
                  text="T-26-4"
                  intname="ussr_t_26_4"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-28_(1938)"
                  title="T-28 (1938)"
                  item_type="own"
                  text="T-28 (1938)"
                  intname="ussr_t_28_1938"
                  type="ground"
                />
                <Arrow length={2} />
              </td>
              <td>
                <TechTreeItem
                  link="/T-60"
                  title="T-60"
                  item_type="own"
                  text="T-60"
                  intname="ussr_t_60_1941"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-70"
                  title="T-70"
                  item_type="own"
                  text="T-70"
                  intname="ussr_t_70_1942"
                  type="ground"
                />
                <Arrow length={3} />
              </td>
              <td>
                <TechTreeItem
                  link="/SU-5-1"
                  title="SU-5-1"
                  item_type="own"
                  text="SU-5-1"
                  intname="ussr_su_5_1"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/SU-76M"
                  title="SU-76M"
                  item_type="own"
                  text="SU-76M"
                  intname="ussr_su_76m_1943"
                  type="ground"
                />
                <Arrow length={3} />
              </td>
              <td>
                <TechTreeItem
                  link="/GAZ-AAA_(4M)"
                  title="GAZ-AAA (4M)"
                  item_type="own"
                  text="GAZ-AAA (4M)"
                  intname="ussr_gaz_4m"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/GAZ-AAA_(DShK)"
                  title="GAZ-AAA (DShK)"
                  item_type="own"
                  text="GAZ-AAA (DShK)"
                  intname="ussr_gaz_dshk"
                  type="ground"
                />
                <Arrow length={3} />
              </td>
              <td style={{ borderLeft: "solid 1px #cccccc" }}>
                <TechTreeItem
                  link="/T-26_(1st_Gv.T.Br.)"
                  title="T-26 (1st Gv.T.Br.)"
                  item_type="prem"
                  text="T-26 (1st Gv.T.Br.)"
                  intname="ussr_t_26_1940_1st_gvtbr"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/T-35"
                  title="T-35"
                  item_type="prem"
                  text="T-35"
                  intname="ussr_t_35"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/SU-57"
                  title="SU-57"
                  item_type="prem"
                  text="SU-57"
                  intname="ussr_su_57"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/T-III_(USSR)"
                  title="T-III (USSR)"
                  item_type="prem"
                  text="▂T-III"
                  intname="ussr_pzkpfw_iii_ausf_j_l42"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/BM-8-24"
                  title="BM-8-24"
                  item_type="prem"
                  text="BM-8-24"
                  intname="ussr_bm_8_24"
                  type="ground"
                />
                <EmptyDiv />
              </td>
              <td>
                <TechTreeItem
                  link="/T-26E"
                  title="T-26E"
                  item_type="prem"
                  text="T-26E"
                  intname="ussr_t_26e"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/BT-7_TD"
                  title="BT-7 TD"
                  item_type="own"
                  text="BT-7 TD"
                  intname="ussr_bt_7_1937_td"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/RBT-5"
                  title="RBT-5"
                  item_type="prem"
                  text="RBT-5"
                  intname="ussr_rbt_5"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/BA-11"
                  title="BA-11"
                  item_type="prem"
                  text="BA-11"
                  intname="ussr_ba_11"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/M3_Medium_(USSR)"
                  title="M3 Medium (USSR)"
                  item_type="prem"
                  text="▂M3 Medium"
                  intname="ussr_m3c"
                  type="ground"
                />
                <EmptyDiv />
              </td>
            </tr>
            <tr>
              <Rank rank="II" />
              <td>
                <TechTreeItem
                  link="/T-50"
                  title="T-50"
                  item_type="own"
                  text="T-50"
                  intname="ussr_t_50"
                  type="ground"
                />
                <Arrow length={0} />
                <TreeFolder name="T-34" img="ussr_t_34_40_41_group">
                  <TechTreeItem
                    link="/T-34_(1940)"
                    title="T-34 (1940)"
                    item_type="own"
                    text="T-34 (1940)"
                    intname="ussr_t_34_1941_l_11"
                    type="ground"
                  />
                  <Arrow length={0} />
                  <TechTreeItem
                    link="/T-34_(1941)"
                    title="T-34 (1941)"
                    item_type="own"
                    text="T-34 (1941)"
                    intname="ussr_t_34_1941"
                    type="ground"
                  />
                </TreeFolder>
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-34_(1942)"
                  title="T-34 (1942)"
                  item_type="own"
                  text="T-34 (1942)"
                  intname="ussr_t_34_1942"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-34E_STZ"
                  title="T-34E STZ"
                  item_type="own"
                  text="T-34E STZ"
                  intname="ussr_t_34e_stz"
                  type="ground"
                />
                <EmptyDiv />
              </td>
              <td>
                <TreeFolder name="T-28/T-28E" img="ussr_t_28_t_28e_group">
                  <TechTreeItem
                    link="/T-28"
                    title="T-28"
                    item_type="own"
                    text="T-28"
                    intname="ussr_t_28"
                    type="ground"
                  />
                  <Arrow length={0} />
                  <TechTreeItem
                    link="/T-28E"
                    title="T-28E"
                    item_type="own"
                    text="T-28E"
                    intname="ussr_t_28e"
                    type="ground"
                  />
                </TreeFolder>
                <Arrow length={0} />
                <TechTreeItem
                  link="/KV-1_(L-11)"
                  title="KV-1 (L-11)"
                  item_type="own"
                  text="KV-1 (L-11)"
                  intname="ussr_kv_1_l_11"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/KV-2_(1939)"
                  title="KV-2 (1939)"
                  item_type="own"
                  text="KV-2 (1939)"
                  intname="ussr_kv_2_1939"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/KV-1S"
                  title="KV-1S"
                  item_type="own"
                  text="KV-1S"
                  intname="ussr_kv_1s"
                  type="ground"
                />
                <EmptyDiv />
              </td>
              <td>
                <TechTreeItem
                  link="/T-80"
                  title="T-80"
                  item_type="own"
                  text="T-80"
                  intname="ussr_t_80"
                  type="ground"
                />
                <Arrow length={6} />
              </td>
              <td>
                <TechTreeItem
                  link="/SU-122"
                  title="SU-122"
                  item_type="own"
                  text="SU-122"
                  intname="ussr_su_122"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/ZiS-30"
                  title="ZiS-30"
                  item_type="own"
                  text="ZiS-30"
                  intname="ussr_zis_30"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/SU-57B"
                  title="SU-57B"
                  item_type="own"
                  text="SU-57B"
                  intname="ussr_su_57b"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/YaG-10_(29-K)"
                  title="YaG-10 (29-K)"
                  item_type="own"
                  text="YaG-10 (29-K)"
                  intname="ussr_zsu_29k"
                  type="ground"
                />
                <Arrow length={3} />
              </td>
              <td>
                <TechTreeItem
                  link="/GAZ-MM_(72-K)"
                  title="GAZ-MM (72-K)"
                  item_type="own"
                  text="GAZ-MM (72-K)"
                  intname="ussr_gaz_mm_72k"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/BTR-152A"
                  title="BTR-152A"
                  item_type="own"
                  text="BTR-152A"
                  intname="ussr_btr_152a"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/ZiS-12_(94-KM)"
                  title="ZiS-12 (94-KM)"
                  item_type="own"
                  text="ZiS-12 (94-KM)"
                  intname="ussr_zis_12_94km_1945"
                  type="ground"
                />
                <Arrow length={4} />
              </td>
              <td style={{ borderLeft: "solid 1px #cccccc" }}>
                <TechTreeItem
                  link="/T-126"
                  title="T-126"
                  item_type="prem"
                  text="T-126"
                  intname="ussr_t_126sp"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/ZUT-37"
                  title="ZUT-37"
                  item_type="prem"
                  text="ZUT-37"
                  intname="ussr_zut_37"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/SMK"
                  title="SMK"
                  item_type="prem"
                  text="SMK"
                  intname="ussr_smk"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/BM-13N"
                  title="BM-13N"
                  item_type="prem"
                  text="BM-13N"
                  intname="ussr_bm_13n"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/MK-IX_%22Valentine%22_(USSR)"
                  title='MK-IX "Valentine" (USSR)'
                  item_type="prem"
                  text='▂МК-IX "Valentine"'
                  intname="ussr_valentine_mk_9"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/T-34_(1st_Gv.T.Br.)"
                  title="T-34 (1st Gv.T.Br.)"
                  item_type="prem"
                  text="T-34 (1st Gv.T.Br.)"
                  intname="ussr_t_34_1941_cast_turret"
                  type="ground"
                />
                <EmptyDiv />
              </td>
              <td>
                <TechTreeItem
                  link="/SU-76M_(5th_Gv.Kav.Corps)"
                  title="SU-76M (5th Gv.Kav.Corps)"
                  item_type="prem"
                  text="SU-76M (5th Gv.Kav.Corps)"
                  intname="ussr_su_76m_5st_kav_corps"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/SU-76D"
                  title="SU-76D"
                  item_type="prem"
                  text="SU-76D"
                  intname="ussr_su_76d"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/BT-7A_(F-32)"
                  title="BT-7A (F-32)"
                  item_type="prem"
                  text="BT-7A (F-32)"
                  intname="ussr_bt_7a_f32"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/MK-II_%22Matilda%22_(USSR)"
                  title='MK-II "Matilda" (USSR)'
                  item_type="prem"
                  text='▂MK-II "Matilda"'
                  intname="ussr_a_12_mk_2_matilda_2a_f96"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/SU-85A"
                  title="SU-85A"
                  item_type="prem"
                  text="SU-85A"
                  intname="ussr_su_85a"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/T-34E"
                  title="T-34E"
                  item_type="prem"
                  text="T-34E"
                  intname="ussr_t_34e"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/T-34_(Prototype)"
                  title="T-34 (Prototype)"
                  item_type="prem"
                  text="T-34 (Prototype)"
                  intname="ussr_t_34_1940_l_11"
                  type="ground"
                />
                <EmptyDiv />
              </td>
            </tr>
            <tr>
              <Rank rank="III" />
              <td>
                <TechTreeItem
                  link="/T-34-57"
                  title="T-34-57"
                  item_type="own"
                  text="T-34-57"
                  intname="ussr_t_34_1941_57"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-34-85_(D-5T)"
                  title="T-34-85 (D-5T)"
                  item_type="own"
                  text="T-34-85 (D-5T)"
                  intname="ussr_t_34_85_d_5t"
                  type="ground"
                />
                <Arrow length={3} />
              </td>
              <td>
                <TechTreeItem
                  link="/KV-1_(ZiS-5)"
                  title="KV-1 (ZiS-5)"
                  item_type="own"
                  text="KV-1 (ZiS-5)"
                  intname="ussr_kv_1_zis_5"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/KV-85"
                  title="KV-85"
                  item_type="own"
                  text="KV-85"
                  intname="ussr_kv_85"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/IS-1"
                  title="IS-1"
                  item_type="own"
                  text="IS-1"
                  intname="ussr_is_1"
                  type="ground"
                />
                <Arrow length={2} />
              </td>
              <td>
                <TechTreeItem
                  link="/ASU-57"
                  title="ASU-57"
                  item_type="own"
                  text="ASU-57"
                  intname="ussr_asu_57"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/PT-76B"
                  title="PT-76B"
                  item_type="own"
                  text="PT-76B"
                  intname="ussr_pt_76b"
                  type="ground"
                />
                <Arrow length={3} />
              </td>
              <td>
                <TreeFolder name="SU-152/ISU-152" img="ussr_su_isu_group">
                  <TechTreeItem
                    link="/SU-152"
                    title="SU-152"
                    item_type="own"
                    text="SU-152"
                    intname="ussr_su_152"
                    type="ground"
                  />
                  <Arrow length={0} />
                  <TechTreeItem
                    link="/ISU-152"
                    title="ISU-152"
                    item_type="own"
                    text="ISU-152"
                    intname="ussr_isu_152"
                    type="ground"
                  />
                </TreeFolder>
                <Arrow length={0} />
                <TreeFolder name="SU-85" img="ussr_su_85_group">
                  <TechTreeItem
                    link="/SU-85"
                    title="SU-85"
                    item_type="own"
                    text="SU-85"
                    intname="ussr_su_85_1943"
                    type="ground"
                  />
                  <Arrow length={0} />
                  <TechTreeItem
                    link="/SU-85M"
                    title="SU-85M"
                    item_type="own"
                    text="SU-85M"
                    intname="ussr_su_85m"
                    type="ground"
                  />
                </TreeFolder>
                <Arrow length={0} />
                <TechTreeItem
                  link="/ISU-122"
                  title="ISU-122"
                  item_type="own"
                  text="ISU-122"
                  intname="ussr_isu_122"
                  type="ground"
                />
                <Arrow length={2} />
              </td>
              <td>
                <TechTreeItem
                  link="/ZiS-43"
                  title="ZiS-43"
                  item_type="own"
                  text="ZiS-43"
                  intname="ussr_zis_43"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/ZSU-37"
                  title="ZSU-37"
                  item_type="own"
                  text="ZSU-37"
                  intname="ussr_zsu_37"
                  type="ground"
                />
                <Arrow length={3} />
              </td>
              <td style={{ borderLeft: "solid 1px #cccccc" }}>
                <TechTreeItem
                  link="/T-34-57_(1943)"
                  title="T-34-57 (1943)"
                  item_type="prem"
                  text="T-34-57 (1943)"
                  intname="ussr_t_34_57_1943"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/SU-100Y"
                  title="SU-100Y"
                  item_type="prem"
                  text="SU-100Y"
                  intname="ussr_su_100y"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/M4A2_(USSR)"
                  title="M4A2 (USSR)"
                  item_type="prem"
                  text="▂M4A2"
                  intname="ussr_m4a2_76w_sherman"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/Ph%C3%B2ng_kh%C3%B4ng_T-34_(USSR)"
                  title="Phòng không T-34 (USSR)"
                  item_type="prem"
                  text="▂Phòng không T-34"
                  intname="ussr_type_65_aa"
                  type="ground"
                />
                <EmptyDiv />
              </td>
              <td>
                <TechTreeItem
                  link="/KV-2_(1940)"
                  title="KV-2 (1940)"
                  item_type="prem"
                  text="KV-2 (1940)"
                  intname="ussr_kv_2_1940"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/KV-1E"
                  title="KV-1E"
                  item_type="prem"
                  text="KV-1E"
                  intname="ussr_kv_1e"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/KV-220"
                  title="KV-220"
                  item_type="prem"
                  text="KV-220"
                  intname="ussr_kv_220"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/KV-2_(ZiS-6)"
                  title="KV-2 (ZiS-6)"
                  item_type="prem"
                  text="KV-2 (ZiS-6)"
                  intname="ussr_kv_2_zis_6"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/T-V_(USSR)"
                  title="T-V (USSR)"
                  item_type="prem"
                  text="▂T-V"
                  intname="ussr_pzkpfw_v"
                  type="ground"
                />
                <EmptyDiv />
              </td>
            </tr>
            <tr>
              <Rank rank="IV" />
              <td>
                <TechTreeItem
                  link="/T-34-85"
                  title="T-34-85"
                  item_type="own"
                  text="T-34-85"
                  intname="ussr_t_34_85_zis_53"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-44"
                  title="T-44"
                  item_type="own"
                  text="T-44"
                  intname="ussr_t_44"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-44-100"
                  title="T-44-100"
                  item_type="own"
                  text="T-44-100"
                  intname="ussr_t_44_100"
                  type="ground"
                />
                <Arrow length={3} />
              </td>
              <td>
                <TechTreeItem
                  link="/IS-2"
                  title="IS-2"
                  item_type="own"
                  text="IS-2"
                  intname="ussr_is_2_1943"
                  type="ground"
                />
                <Arrow length={0} />
                <TreeFolder name="IS-2 (1944)" img="ussr_is_2_1944_group">
                  <TechTreeItem
                    link="/IS-2_(1944)"
                    title="IS-2 (1944)"
                    item_type="own"
                    text="IS-2 (1944)"
                    intname="ussr_is_2_1944"
                    type="ground"
                  />
                  <div
                    style={{
                      position: "relative",
                      width: 160,
                      height: 30,
                      margin: "auto",
                    }}
                  />
                  <TechTreeItem
                    link="/IS-2_No.321"
                    title="IS-2 No.321"
                    item_type="own"
                    text="IS-2 No.321"
                    intname="ussr_is_2_1944_321"
                    type="ground"
                  />
                </TreeFolder>
                <Arrow length={4} />
              </td>
              <td>
                <TechTreeItem
                  link="/ASU-85"
                  title="ASU-85"
                  item_type="own"
                  text="ASU-85"
                  intname="ussr_asu_85"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/SU-100P"
                  title="SU-100P"
                  item_type="own"
                  text="SU-100P"
                  intname="ussr_su_100p"
                  type="ground"
                />
                <Arrow length={4} />
              </td>
              <td>
                <TechTreeItem
                  link="/ISU-122S"
                  title="ISU-122S"
                  item_type="own"
                  text="ISU-122S"
                  intname="ussr_isu_122s"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/SU-100"
                  title="SU-100"
                  item_type="own"
                  text="SU-100"
                  intname="ussr_su_100_1945"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/2S3M"
                  title="2S3M"
                  item_type="own"
                  text="2S3M"
                  intname="ussr_2s3m"
                  type="ground"
                />
                <Arrow length={3} />
              </td>
              <td>
                <TechTreeItem
                  link="/BTR-ZD"
                  title="BTR-ZD"
                  item_type="own"
                  text="BTR-ZD"
                  intname="ussr_btr_zd"
                  type="ground"
                />
                <Arrow length={5} />
              </td>
              <td style={{ borderLeft: "solid 1px #cccccc" }}>
                <TechTreeItem
                  link="/T-34-85E"
                  title="T-34-85E"
                  item_type="prem"
                  text="T-34-85E"
                  intname="ussr_t_34_85e"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/SU-122P"
                  title="SU-122P"
                  item_type="prem"
                  text="SU-122P"
                  intname="ussr_su_122p"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/T-44-122"
                  title="T-44-122"
                  item_type="prem"
                  text="T-44-122"
                  intname="ussr_t_44_122"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/T-34-100"
                  title="T-34-100"
                  item_type="prem"
                  text="T-34-100"
                  intname="ussr_t_34_100"
                  type="ground"
                />
                <EmptyDiv />
              </td>
              <td>
                <TechTreeItem
                  link="/KV-122"
                  title="KV-122"
                  item_type="prem"
                  text="KV-122"
                  intname="ussr_kv_122"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/IS-2_%22Revenge%22"
                  title='IS-2 "Revenge"'
                  item_type="prem"
                  text='IS-2 "Revenge"'
                  intname="ussr_is_2_1944_revenge"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/Object_248"
                  title="Object 248"
                  item_type="prem"
                  text="Object 248"
                  intname="ussr_object_248"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/IS-6"
                  title="IS-6"
                  item_type="prem"
                  text="IS-6"
                  intname="ussr_is_6"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/Type_62_(USSR)"
                  title="Type 62 (USSR)"
                  item_type="own"
                  text="▂Type 62"
                  intname="ussr_type_62"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/PT-76-57"
                  title="PT-76-57"
                  item_type="prem"
                  text="PT-76-57"
                  intname="ussr_pt_76_57"
                  type="ground"
                />
                <EmptyDiv />
              </td>
            </tr>
            <tr>
              <Rank rank="V" />
              <td>
                <TechTreeItem
                  link="/T-54_(1949)"
                  title="T-54 (1949)"
                  item_type="own"
                  text="T-54 (1949)"
                  intname="ussr_t_54_1949"
                  type="ground"
                />
                <Arrow length={0} />
                <TreeFolder name="T-54" img="ussr_t_54_group">
                  <TechTreeItem
                    link="/T-54_(1947)"
                    title="T-54 (1947)"
                    item_type="own"
                    text="T-54 (1947)"
                    intname="ussr_t_54_1947"
                    type="ground"
                  />
                  <Arrow length={0} />
                  <TechTreeItem
                    link="/T-54_(1951)"
                    title="T-54 (1951)"
                    item_type="own"
                    text="T-54 (1951)"
                    intname="ussr_t_54_1951"
                    type="ground"
                  />
                </TreeFolder>
                <Arrow length={0} />
              </td>
              <td>
                <TechTreeItem
                  link="/IS-3"
                  title="IS-3"
                  item_type="own"
                  text="IS-3"
                  intname="ussr_is_3"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/IS-4M"
                  title="IS-4M"
                  item_type="own"
                  text="IS-4M"
                  intname="ussr_is_4m"
                  type="ground"
                />
                <Arrow length={0} />
              </td>
              <td>
                <TechTreeItem
                  link="/BMP-1"
                  title="BMP-1"
                  item_type="own"
                  text="BMP-1"
                  intname="ussr_bmp_1"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/Object_906"
                  title="Object 906"
                  item_type="own"
                  text="Object 906"
                  intname="ussr_object_906"
                  type="ground"
                />
                <Arrow length={0} />
              </td>
              <td>
                <TechTreeItem
                  link="/SU-122-54"
                  title="SU-122-54"
                  item_type="own"
                  text="SU-122-54"
                  intname="ussr_su_122_54"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/Object_268"
                  title="Object 268"
                  item_type="own"
                  text="Object 268"
                  intname="ussr_object_268"
                  type="ground"
                />
                <Arrow length={0} />
              </td>
              <td>
                <TechTreeItem
                  link="/ZSU-57-2"
                  title="ZSU-57-2"
                  item_type="own"
                  text="ZSU-57-2"
                  intname="ussr_zsu_57_2"
                  type="ground"
                />
                <Arrow length={1} />
              </td>
              <td style={{ borderLeft: "solid 1px #cccccc" }}>
                <TechTreeItem
                  link="/Object_120"
                  title="Object 120"
                  item_type="prem"
                  text="Object 120"
                  intname="ussr_object_120"
                  type="ground"
                />
                <EmptyDiv />
              </td>
              <td>
                <TechTreeItem
                  link="/T-10A"
                  title="T-10A"
                  item_type="prem"
                  text="T-10A"
                  intname="ussr_t_10a"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/IS-7"
                  title="IS-7"
                  item_type="own"
                  text="IS-7"
                  intname="ussr_is_7"
                  type="ground"
                />
                <EmptyDiv />
              </td>
            </tr>
            <tr>
              <Rank rank="VI" />
              <td>
                <TechTreeItem
                  link="/T-55A"
                  title="T-55A"
                  item_type="own"
                  text="T-55A"
                  intname="ussr_t_55a"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-55AMD-1"
                  title="T-55AMD-1"
                  item_type="own"
                  text="T-55AMD-1"
                  intname="ussr_t_55_amd_1"
                  type="ground"
                />
                <Arrow length={0} />
                <TreeFolder name="T-62" img="t_62_group">
                  <TechTreeItem
                    link="/T-62"
                    title="T-62"
                    item_type="own"
                    text="T-62"
                    intname="ussr_t_62"
                    type="ground"
                  />
                  <Arrow length={0} />
                  <TechTreeItem
                    link="/T-62M-1"
                    title="T-62M-1"
                    item_type="own"
                    text="T-62M-1"
                    intname="ussr_t_62m1"
                    type="ground"
                  />
                </TreeFolder>
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-72A"
                  title="T-72A"
                  item_type="own"
                  text="T-72A"
                  intname="ussr_t_72a"
                  type="ground"
                />
                <Arrow length={0} />
                <TreeFolder name="T-72B" img="t_72b_group">
                  <TechTreeItem
                    link="/T-72B"
                    title="T-72B"
                    item_type="own"
                    text="T-72B"
                    intname="ussr_t_72b"
                    type="ground"
                  />
                  <Arrow length={0} />
                  <TechTreeItem
                    link="/T-72B_(1989)"
                    title="T-72B (1989)"
                    item_type="own"
                    text="T-72B (1989)"
                    intname="ussr_t_72b_1989"
                    type="ground"
                  />
                </TreeFolder>
                <Arrow length={0} />
              </td>
              <td>
                <TechTreeItem
                  link="/T-10M"
                  title="T-10M"
                  item_type="own"
                  text="T-10M"
                  intname="ussr_t_10m"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-64A_(1971)"
                  title="T-64A (1971)"
                  item_type="own"
                  text="T-64A (1971)"
                  intname="ussr_t_64a_1971"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-64B"
                  title="T-64B"
                  item_type="own"
                  text="T-64B"
                  intname="ussr_t_64_b_1984"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-80B"
                  title="T-80B"
                  item_type="own"
                  text="T-80B"
                  intname="ussr_t_80b"
                  type="ground"
                />
                <Arrow length={1} />
              </td>
              <td>
                <TechTreeItem
                  link="/BMP-2"
                  title="BMP-2"
                  item_type="own"
                  text="BMP-2"
                  intname="ussr_bmp_2"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/BMP-3"
                  title="BMP-3"
                  item_type="own"
                  text="BMP-3"
                  intname="ussr_bmp_3"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/Object_685"
                  title="Object 685"
                  item_type="own"
                  text="Object 685"
                  intname="ussr_object_685"
                  type="ground"
                />
                <Arrow length={2} />
              </td>
              <td>
                <TechTreeItem
                  link="/IT-1"
                  title="IT-1"
                  item_type="own"
                  text="IT-1"
                  intname="ussr_it_1"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/Shturm-S"
                  title="Shturm-S"
                  item_type="own"
                  text="Shturm-S"
                  intname="ussr_9p149"
                  type="ground"
                />
                <Arrow length={3} />
              </td>
              <td>
                <TechTreeItem
                  link="/ZSU-37-2"
                  title="ZSU-37-2"
                  item_type="own"
                  text="ZSU-37-2"
                  intname="ussr_zsu_37_2"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/ZSU-23-4"
                  title="ZSU-23-4"
                  item_type="own"
                  text="ZSU-23-4"
                  intname="ussr_zsu_23_4"
                  type="ground"
                />
                <Arrow length={3} />
              </td>
              <td style={{ borderLeft: "solid 1px #cccccc" }}>
                <TechTreeItem
                  link="/T-55AM-1"
                  title="T-55AM-1"
                  item_type="prem"
                  text="T-55AM-1"
                  intname="ussr_t_55_am"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/T-72AV_(TURMS-T)"
                  title="T-72AV (TURMS-T)"
                  item_type="prem"
                  text="T-72AV (TURMS-T)"
                  intname="ussr_t_72av_turms"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/2S38"
                  title="2S38"
                  item_type="prem"
                  text="2S38"
                  intname="ussr_2s38"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/BMP-2M"
                  title="BMP-2M"
                  item_type="squad"
                  text="BMP-2M"
                  intname="ussr_bmp_2m"
                  type="ground"
                />
              </td>
              <td>
                <TechTreeItem
                  link="/Object_279"
                  title="Object 279"
                  item_type="own"
                  text="Object 279"
                  intname="ussr_object_279"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/BMD-4"
                  title="BMD-4"
                  item_type="own"
                  text="BMD-4"
                  intname="ussr_bmd_4"
                  type="ground"
                />
                <EmptyDiv />
                <TechTreeItem
                  link="/T-72M2_Moderna"
                  title="T-72M2 Moderna"
                  item_type="own"
                  text="T-72M2 Moderna"
                  intname="ussr_t_72m2_moderna"
                  type="ground"
                />
              </td>
            </tr>
            <tr>
              <Rank rank="VII" />
              <td>
                <TechTreeItem
                  link="/T-90A"
                  title="T-90A"
                  item_type="own"
                  text="Т-90А"
                  intname="ussr_t_90a"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-72B3"
                  title="T-72B3"
                  item_type="own"
                  text="T-72B3"
                  intname="ussr_t_72b3_2011"
                  type="ground"
                />
                <EmptyDiv />
              </td>
              <td>
                <TechTreeItem
                  link="/T-80U"
                  title="T-80U"
                  item_type="own"
                  text="T-80U"
                  intname="ussr_t_80u"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/T-80BVM"
                  title="T-80BVM"
                  item_type="own"
                  text="T-80BVM"
                  intname="ussr_t_80bvm"
                  type="ground"
                />
                <EmptyDiv />
              </td>
              <td>
                <TechTreeItem
                  link="/2S25"
                  title="2S25"
                  item_type="own"
                  text="2S25"
                  intname="ussr_2s25"
                  type="ground"
                />
                <Arrow length={0} />
                <TechTreeItem
                  link="/2S25M"
                  title="2S25M"
                  item_type="own"
                  text="2S25M"
                  intname="ussr_2s25m"
                  type="ground"
                />
                <EmptyDiv />
              </td>
              <td>
                <TechTreeItem
                  link="/Khrizantema-S"
                  title="Khrizantema-S"
                  item_type="own"
                  text="Khrizantema-S"
                  intname="ussr_9p157"
                  type="ground"
                />
                <EmptyDiv />
              </td>
              <td>
                <TechTreeItem
                  link="/2S6"
                  title="2S6"
                  item_type="own"
                  text="2S6"
                  intname="ussr_zprk_2s6"
                  type="ground"
                />
                <EmptyDiv />
              </td>
              <td style={{ borderLeft: "solid 1px #cccccc" }}>
                <TechTreeItem
                  link="/T-80UK"
                  title="T-80UK"
                  item_type="squad"
                  text="T-80UK"
                  intname="ussr_t_80uk"
                  type="ground"
                />
              </td>
              <td>
                <EmptyDiv />
              </td>
            </tr>
          </tbody>
        </table>
      </div>
      <MediaHead />
      <div className="video_gallery">
        <a href="https://yewtu.be/4tLiivpJ7nk">
          <b>Book of Records USSR</b> - <i>War Thunder Official Channel</i>
        </a>
        <a href="https://yewtu.be/klfkO6hJQGk">
          <b>Climbing the ranks with Soviet tanks</b> - <i>War Thunder Official Channel</i>
        </a>
        <a href="https://yewtu.be/z5ZZzHNPYi4">
          <b>Russian ground forces Tier I - Review and Analysis</b> - <i>Oxy</i>
        </a>
        <a href="https://yewtu.be/TXWa-Nnc6IE">
          <b>Russian ground forces Tier II - Review and Analysis</b> - <i>Oxy</i>
        </a>
        <a href="https://yewtu.be/MRlEuQRfHdk">
          <b>Russian ground forces Tier III - Review and Analysis</b> - <i>Oxy</i>
        </a>
        <a href="https://yewtu.be/nw2LFVdhm5s">
          <b>Russian ground forces Tier IV - Review and Analysis</b> - <i>Oxy</i>
        </a>
        <a href="https://yewtu.be/XtuTkDhE3OQ">
          <b>Russian ground forces Tier V - Review and Analysis</b> - <i>Oxy</i>
        </a>
        <a href="https://yewtu.be/idwfz6_Nl6I">
          <b>Russian ground forces Tier VI/VII - Review and Analysis</b> - <i>Oxy</i>
        </a>
      </div>
    </div>
  );
}
